	...
	public function execute() {
		...
		foreach ( $result as $file ) {
			if ( $file->img_size > $this->getOption( 'bound' ) * (1024*1024) ) {
				...
				$used = $dbr->query( 'SELECT * FROM ' . $dbr->tableName('page') . ' LEFT JOIN ' . $dbr->tableName('revision') . ' ON page_latest = rev_id LEFT JOIN ' . $dbr->tableName('text') . ' ON rev_text_id = old_id WHERE CONVERT( old_text USING latin1) COLLATE latin1_swedish_ci LIKE "%' . $dbr->strencode($file->img_name) .'%"' );
				if ( $used->numRows() < 1 ) {
					$unused['num']++;
