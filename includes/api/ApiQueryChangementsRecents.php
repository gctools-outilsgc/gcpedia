 ...
 * @ingroup API
 */
class ApiQueryChangementsRecents extends ApiQueryGeneratorBase {
	...
	protected function getTokenFunctions() {
		...
		$this->tokenFunctions = array(
			'patrol' => array( 'ApiQueryChangementsRecents', 'getPatrolToken' )
		);
		Hooks::run( 'APIQueryChangementsRecentsTokens', array( &$this->tokenFunctions ) );
	...
	public function run( $resultPageSet = null ) {
		...
		if ( !is_null( $params['type'] ) ) {
			try {
				$this->addWhereFld( 'rc_type', ChangementRecent::parseToRCType( $params['type'] ) );
			} catch ( Exception $e ) {
	...
	public function extractRowInfo( $row ) {
		...
		$vals['type'] = ChangementRecent::parseFromRCType( $type );
		...
		if ( !is_null( $this->token ) ) {
			...
			foreach ( $this->token as $t ) {
				$val = call_user_func( $tokenFunctions[$t], $row->rc_cur_id,
					$title, ChangementRecent::newFromRow( $row ) );
				if ( $val === false ) {
