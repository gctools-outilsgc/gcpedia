...
class RandomImage {
	...
	public function render() {
		...
		if( $title instanceof Title && $this->imageExists( $title ) ) {
			return str_ireplace('</body></html>', '', str_ireplace('<html><body>', '', str_ireplace('<p>', '', str_ireplace('<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN" "http://www.w3.org/TR/REC-html40/loose.dtd">', '', $this->removeMagnifier( 
	...
	protected function buildMarkup( $title ) {
		...
		if( stripos( $this->caption, '|' ) >= 0 )
			$ctext = substr( $this->caption, 0, stripos( $this->caption, '|' ) );
		else
			$ctext = $this->caption;
		
		return "<div style='width:200px; float:left; padding:10px 0 5px 10px; margin-top:-25px;'>[[" . implode( "|", $parts ) . "|". $this->caption ."]]</div><div style='width:60%; float:left; font-size:200%; line-height:100%; padding:10px 0 10px 20px'>". $ctext ."</div>";
	}
/**
 *  <div style="width:30%; margin: 5px 0 5px 8px;">[[' . implode( '|', $parts ) . '|'. $this->caption .']]</div><div style="float:left; margin:0 0 0 0; padding: 30px 150px 25px 50px; font-size:200%">'. $this->caption .'</div>'
**/
	/**
