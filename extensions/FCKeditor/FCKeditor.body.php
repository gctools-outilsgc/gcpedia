...
class FCKeditor_MediaWiki {
	...
	public function onEditPageShowEditFormInitial( $form ) {
		...
		if( !isset( $this->showFCKEditor ) ){
			...
			// always start disabled to prevent problems in MW1.18
		...
		// Don't remember state -> prevents breaking in MW1.18
...
// Set config
var wgFCKEditorDir = "' . $wgFCKEditorDir . '";
var wgFCKEditorExtDir = "' . $wgFCKEditorExtDir . '";
var wgFCKEditorToolbarSet = "' . $wgFCKEditorToolbarSet . '";
var wgFCKEditorHeight = "' . $wgFCKEditorHeight . '";
...
function onLoadFCKeditor(){
	...
	if ( realTextarea ){
		...
		if ( height == 0 ){
			...
			var offset = document.getElementById( 'wikiPreview' ) || document.getElementById( 'toolbar' ) || document.getElementById( 'wikiEditor-ui-toolbar' );
			while ( offset ){
		...
		try{
			...
			document.getElementById( 'wikiEditor-ui-toolbar' ).style.display = 'none';
		}
...
function initEditor(){
	...
	if ( toolbar == null )		// if an extension (ex. WikiEditor) is replacing the default toolbar
...
function ToggleFCKEditor( mode, objId ){
	...
	if ( firstLoad ){
		...
		if ( FCKtoolbar == null )		// if an extension (ex. WikiEditor) is replacing the default toolbar
			FCKtoolbar = document.getElementById( 'wikiEditor-ui-toolbar' );
